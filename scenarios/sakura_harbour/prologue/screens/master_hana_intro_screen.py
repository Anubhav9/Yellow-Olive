...
from scenarios.sakura_harbour.prologue.dialogues import master_hana_intro_dialogue
from utils import general_utils


class MasterHanaIntroScreen(RichLog):
    can_focus = True
    ...
    def on_mount(self) -> None:
        self.focus()
        self.run_worker(
            self.render_master_hana_intro(global_constants.GLOBAL_DIALOGUE_COLOR)
        )
    ...
    async def render_master_hana_intro(self, color: str) -> None:
        ...
        image_path = Path(global_constants.IMAGE_MEDIA_PATH) / "hana.png"
    ...
    async def on_key(self, event: events.Key) -> None:
        if event.key == "enter":
            ...
            general_utils.update_progress(
                story_intro_act=global_constants.STORY_ACT_SAKURA_GATE,
            )
            ...
            await container.mount(GateThreeIntroScreen())
