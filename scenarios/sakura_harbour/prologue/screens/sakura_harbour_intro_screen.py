...
from scenarios.sakura_harbour.prologue.dialogues import sakura_harbour_intro_dialogue
from services import resource_manager
from utils import general_utils


class SakuraHarbourIntroScreen(RichLog):
    can_focus = True
    ...
    def on_mount(self) -> None:
        self.focus()
        self.run_worker(self._bootstrap_sakura_harbour_infra(), exclusive=False)
        self.run_worker(
            self.render_sakura_harbour_intro(global_constants.GLOBAL_DIALOGUE_COLOR)
        )

    async def _bootstrap_sakura_harbour_infra(self) -> None:
        await asyncio.to_thread(
            resource_manager.apply_prologue_resources, "sakura_harbour"
        )
    ...
    async def render_sakura_harbour_intro(self, color: str) -> None:
        ...
        dialogues = sakura_harbour_intro_dialogue.SAKURA_HARBOUR_INTRO_DIALOGUES.replace(
    ...
    async def on_key(self, event: events.Key) -> None:
        if event.key == "enter":
            ...
            general_utils.update_progress(
                story_intro_act=global_constants.STORY_ACT_SAKURA_HANA,
            )
            ...
            await container.mount(MasterHanaIntroScreen())
