...
from scenarios.gold_rush_city.prologue.dialogues import team_evil_license_intro_dialogue
from utils import general_utils


class TeamEvilLicenseIntroScreen(RichLog):
    can_focus = True
    ...
    def on_mount(self) -> None:
        self.focus()
        self.run_worker(
            self.render_team_evil_license_intro(
                global_constants.GLOBAL_DIALOGUE_COLOR
            )
        )

    async def render_team_evil_license_intro(self, color: str) -> None:
        ...
        dialogues = team_evil_license_intro_dialogue.TEAM_EVIL_LICENSE_INTRO_DIALOGUES.replace(
    ...
    async def on_key(self, event: events.Key) -> None:
        if event.key == "enter":
            ...
            general_utils.update_progress(
                active_challenge_id="14",
                story_intro_act=global_constants.STORY_ACT_DONE,
            )
            container = self.parent
            await self.remove()
            challenge_14 = general_utils.load_challenge("14")
            await container.mount(challenge_14())
