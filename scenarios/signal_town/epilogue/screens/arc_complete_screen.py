...
from scenarios.signal_town.epilogue.dialogues import arc_complete_dialogue
from utils import general_utils


class SignalTownArcCompleteScreen(RichLog):
    can_focus = True
    ...
    async def render_arc_complete(self, color: str) -> None:
        ...
        dialogues = arc_complete_dialogue.SIGNAL_TOWN_ARC_COMPLETE_DIALOGUES.replace(
        ...
        title = global_constants.ARC_TITLE_SIGNAL_TOWN
        ...
        self.write(
            "\n[bold #D4AF37]The road west leads to Gold Rush City.[/]"
        )
    ...
    async def on_key(self, event: events.Key) -> None:
        if event.key == "enter":
            ...
            from scenarios.gold_rush_city.prologue.screens.gold_rush_city_intro_screen import (
                GoldRushCityIntroScreen,
            )

            await container.mount(GoldRushCityIntroScreen())
