...
from scenarios.signal_town.prologue.dialogues import signal_town_intro_dialogue
...
class SignalTownIntroScreen(RichLog):
    ...
    def on_mount(self) -> None:
        self.focus()
        self.run_worker(self._bootstrap_signal_town_infra(), exclusive=False)
        self.run_worker(
            self.render_signal_town_intro(global_constants.GLOBAL_DIALOGUE_COLOR)
        )

    async def _bootstrap_signal_town_infra(self) -> None:
        """Apply signal-town's prologue resources (namespace) on entering the arc.

        Cluster is already up by the time this screen mounts (started during
        the oakwood prologue or on resume), so we just apply directly."""
        await asyncio.to_thread(
            resource_manager.apply_prologue_resources, "signal_town"
        )
