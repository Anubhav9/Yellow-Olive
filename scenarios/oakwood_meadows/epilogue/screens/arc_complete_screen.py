...
from scenarios.oakwood_meadows.epilogue.dialogues import arc_complete_dialogue
from utils import general_utils


class OakwoodMeadowsArcCompleteScreen(RichLog):
    can_focus = True
    ...
    async def render_arc_complete(self, color: str) -> None:
        ...
        dialogues = arc_complete_dialogue.OAKWOOD_MEADOWS_ARC_COMPLETE_DIALOGUES.replace(
        ...
        title = global_constants.ARC_TITLE_OAKWOOD_MEADOWS
        ...
        self.write(
            "\n[bold #D4AF37]The road beyond the laboratory leads to Signal Town.[/]"
        )
    ...
    async def on_key(self, event: events.Key) -> None:
        if event.key == "enter":
            ...
            general_utils.update_progress(
                section_completed="oakwood_meadows_epilogue",
                section_scenario="oakwood_meadows",
                pending_epilogue=None,
            )
            container = self.parent
            await self.remove()
            from scenarios.signal_town.prologue.screens.signal_town_intro_screen import (
                SignalTownIntroScreen,
            )
