...
class ChallengeMusicPreferenceScreen(Static):
    ...
    def compose(self) -> ComposeResult:
        yield Label(
            f"[bold {global_constants.GLOBAL_DIALOGUE_COLOR}]Professor Bald[/]"
        )
        yield Label(
            f"[bold {global_constants.GLOBAL_DIALOGUE_COLOR}]"
            "Some trainers think better in silence; others need the battle hum.[/]"
        )
        yield Label(
            f"[bold {global_constants.GLOBAL_DIALOGUE_COLOR}]"
            "Keep music on during challenges—everything else stays as before?[/]"
        )
        yield Input(
