...
from challenge_files import challenge_10_text
...
class Challenge10(BaseChallengeScreen):
    challenge_id = "10"
    challenge_text = challenge_10_text.CHALLENGE_10_TEXT
    ...
    def apply_challenge_pod(self) -> None:
        try:
            ...
            apply_commands = (
                ...
                f'kubectl delete pod trainer-relay-pod -n default --ignore-not-found=true && '
                f'kubectl apply -f "{relay_pod_path}"'
            )
            subprocess.Popen(
                ["sh", "-c", apply_commands],
            ...
            log.write("[red]Failed to start challenge 10 setup[/red]")
            log.write(str(error))
