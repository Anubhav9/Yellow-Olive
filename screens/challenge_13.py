...
from challenge_files import challenge_13_text
...
class Challenge13(BaseChallengeScreen):
    challenge_id = "13"
    challenge_text = challenge_13_text.CHALLENGE_13_TEXT
    ...
    def apply_challenge_pod(self) -> None:
        try:
            ...
            ingress_path = general_utils.get_lab_ingress_file(self.challenge_id)
            apply_commands = (
                f'kubectl apply -f "{namespace_path}" && '
                f'kubectl delete pods,service,ingress --all -n signal-town --ignore-not-found=true && '
                f'kubectl apply -f "{bulba_pod_path}" && '
                f'kubectl apply -f "{service_path}" && '
                f'kubectl apply -f "{ingress_path}"'
            )
            subprocess.Popen(
                ["sh", "-c", apply_commands],
            ...
            log.write("[red]Failed to start challenge 13 setup[/red]")
            log.write(str(error))
